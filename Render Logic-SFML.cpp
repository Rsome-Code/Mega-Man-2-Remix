...
public:
	...
	renderer(Vector2i size, bool fullScreen) {
		if (fullScreen) {
			w = shared_ptr<sf::RenderWindow>(new sf::RenderWindow(sf::VideoMode(size.x, size.y), "Executable", sf::Style::Fullscreen));
		}
	}

	renderer(shared_ptr<sf::RenderWindow> wi, bool fullscreen) {
		this->w = wi;
		ratio = getRatio();
	...
	void setSize(sf::Vector2u size) {
		...
		if (fullscreen) {
			...
			w = shared_ptr<sf::RenderWindow>(new sf::RenderWindow(sf::VideoMode(size.x, size.y), "Executable", sf::Style::Default));
		}
...
private:
	...
	void resolutionFix(shared_ptr<RectangleShape> rect) {
		...
		//float resolutionFactor = float(windowS) / float(defaultResolution);
		rect->setScale(Maths::mult(rect->getScale(), resolutionFactor));
		rect->setPosition(Maths::mult(rect->getPosition(), resolutionFactor));
	}
	...
	void rectCorrection(shared_ptr<RectangleShape> rect) {
		...
		//float resolutionFactor = float(windowS) / float(defaultResolution);
		rect->setScale(Maths::div(rect->getScale(), resolutionFactor));
		rect->setPosition(Maths::div(rect->getPosition(), resolutionFactor));
	}

	void resolutionFix(shared_ptr<sf::Text> t) {

		int windowS = w->getSize().y;

		//float resolutionFactor = float(windowS) / float(defaultResolution);

		t->setScale(sf::Vector2f(1 * resolutionFactor, 1 * resolutionFactor));

		Vector2f newPos = Vector2f(Maths::mult(Vector2f(t->getPosition().x, t->getPosition().y), resolutionFactor));
		t->setPosition(sf::Vector2f(newPos.x, newPos.y));
	}
	...
	void resolutionFix(shared_ptr<text> te) {
		...
		shared_ptr<sf::Text> t = te->getRender();

		t->setScale(sf::Vector2f(1 * resolutionFactor, 1 * resolutionFactor));

		Vector2f newPos = Vector2f(Maths::mult(te->getPosition(), resolutionFactor));
		t->setPosition(sf::Vector2f(newPos.x, newPos.y));
	}

	void textCorrection(shared_ptr<sf::Text> t) {
		int windowS = w->getSize().y;
...
public:
	...
	void textDisplay(shared_ptr<sf::Text> text) {
		//list<shared_ptr<text>> temp = resolutionTextScale(textList);
		resolutionFix(text);
		w->draw(*text);
		textCorrection(text);
	...
	void textDisplay(vector<shared_ptr<sf::Text>> text) {

		for (shared_ptr<sf::Text> t : text) {
			textDisplay(t);
		}
	}
	...
	void objectSetup(shared_ptr<objectSprite> object, shared_ptr<camera> cam) {
		...
		Vector2f newP = Maths::sub(sPosition, cPosition);
		Vector2f plusZ = Vector2f((newP.x * pow(object->getZ(), -1)), (newP.y * pow(object->getZ(), -1)));

		Vector2f newPos = Maths::add(object->getVisualOffset(), plusZ);
	...
	void UIDisplay(shared_ptr<UISprite> sprite) {
		...
		Vector2f newPos = Maths::sub(sprite->getCameraPosition(), sprite->getVisualOffset());
	...
	void hitboxDisplay(shared_ptr<objectHitbox> hit) {
		...
		//if (hit->isVisible()) {
		RectangleShape rectangle(Vector2f(hit->getSize().x * ratio, hit->getSize().y * ratio));
		rectangle.setPosition(hit->getCameraPos());
		rectangle.setFillColour(Colour(0, 255, 0, 255));

		resolutionFix(hit);
		w->draw(*rectangle.getShape());
		//}
