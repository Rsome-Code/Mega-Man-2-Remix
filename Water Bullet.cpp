...
public:
	void spriteSetup(Texture* t, Vector2f position) {
		mov = new movable(t, IntRect(39, 86, 12, 6), position, Vector2f(4, 4));
		sprite = mov;
		anim = new animation(list<IntRect>{IntRect(39, 86, 16, 6), IntRect(56, 87, 11, 6), IntRect(73, 88, 12, 6)}, sprite);
		if (angle == 0) {
			anim->swapAll();
